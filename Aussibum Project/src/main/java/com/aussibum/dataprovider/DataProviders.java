package com.aussibum.dataprovider;
...
import com.aussibum.utility.ExcelLibrary;
...
public class DataProviders {
	...
	public Object[][] getcheckout_data() {
		// Totals rows count
		int rows = obj.getRowCount("Checkout_Form");
		// Total Columns
		int column = obj.getColumnCount("Checkout_Form");
		int actRows = rows - 1;
		...
		for (int i = 0; i < actRows; i++) {
			for (int j = 0; j < column; j++) {
				data[i][j] = obj.getCellData("Checkout_Form", j, i + 2);
			}
		}
		return data;
	}
